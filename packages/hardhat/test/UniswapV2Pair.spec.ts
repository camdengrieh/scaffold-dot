...
describe("UniswapV2Pair", () => {
  async function fixture() {
    ...
    const factory = await (await ethers.getContractFactory("UniswapV2Factory")).deploy(wallet.address);

    const tokenA = (await erc20Factory.deploy(expandTo18Decimals(10000))) as ERC20;
    const tokenB = (await erc20Factory.deploy(expandTo18Decimals(10000))) as ERC20;
    ...
    const pair = pairFactory.attach(await factory.getPair(tokenAAddress, tokenBAddress)) as UniswapV2Pair;
  ...
  optimisticTestCases.forEach((optimisticTestCase, i) => {
    it(`optimistic:${i}`, async () => {
      ...
      await expect(pair.swap(outputAmount + 1n, 0n, wallet.address, "0x")).to.be.revertedWith("UniswapV2: K");
  ...
  it("swap:gas", async () => {
    ...
    await ethers.provider.send("evm_mine", [(await wallet.provider.getBlock("latest"))!.timestamp + 1]);
